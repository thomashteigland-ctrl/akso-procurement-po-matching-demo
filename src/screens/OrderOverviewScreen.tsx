...
import { docTypeLabel, docTypeShort, formatNok } from '../lib/format'
import { STAGE_FROM_DOC, STAGE_LABEL, buildOrders, stageIndex } from '../lib/orders'
import { useDemoStore } from '../store/demoStore'
import type { DocType, Order, OrderStage } from '../types'
...
function OrderDetail({ order }: { order: Order }) {
  ...
  const byType = new Map(order.documents.map((doc) => [doc.docType, doc]))
    ...
    <div className="flex min-h-0 flex-1 flex-col overflow-auto">
      <div className="mb-3 flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-ink-soft">{order.supplier}</p>
        </div>
      ...
      <dl className="mb-4 grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">
        ...
        <div className="rounded-lg bg-paper px-3 py-2">
          ...
          <dd className="font-medium text-ink">{formatNok(order.amount)}</dd>
        </div>
        <div className="rounded-lg bg-paper px-3 py-2">
          <dt className="text-[11px] text-muted">Documents</dt>
          <dd className="font-medium text-ink">{order.documents.length} of 4</dd>
        </div>
      </dl>

      <p className="mb-2 text-[11px] font-semibold tracking-wide text-muted uppercase">
        Amounts across documents
      </p>
      <div className="mb-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
        {COLS.map((type) => {
          const doc = byType.get(type)
          const off = Boolean(doc && doc.amount !== order.amount)
          return (
            <div
              key={type}
              className={`rounded-lg px-3 py-2 ${
                off ? 'bg-amber-light/80 ring-1 ring-amber/30' : 'bg-paper'
              }`}
            >
              <p className="text-[11px] text-muted">{docTypeLabel(type)}</p>
              <p className={`font-mono text-sm font-medium ${off ? 'text-amber' : 'text-ink'}`}>
                {doc ? formatNok(doc.amount) : '—'}
              </p>
            </div>
          )
        })}
        ...
        <table className="w-full text-left text-xs">
          <thead className="bg-paper text-[10px] tracking-wide text-muted uppercase">
            <tr>
              ...
              {COLS.map((type) => (
                <th key={type} className="px-3 py-2 text-right font-medium">
                  {docTypeShort(type)} qty
                </th>
              ))}
            </tr>
          </thead>
          <tbody>
            {order.lineItems.map((line) => (
              ...
              >
                <td className="px-3 py-1.5 font-medium text-ink">{line.product}</td>
                {COLS.map((type) => {
                  const cell = line.byType[type]
                  return (
                    <td
                      key={type}
                      className={`px-3 py-1.5 text-right font-mono ${line.mismatch ? 'text-amber' : 'text-ink-soft'}`}
                    >
                      {cell ? cell.quantity.toLocaleString('nb-NO') : '—'}
                    </td>
                  )
                })}
              </tr>
      ...
      <div className="mb-3 flex flex-wrap gap-2">
        {COLS.map((type) => {
          ...
          return (
            ...
            >
              <span className="font-semibold">{docTypeLabel(type)}</span>
    ...
    <div className="flex h-full min-h-0 flex-col p-5">
      <div className="mb-3">
        ...
        <p className="text-sm text-muted">
          Each purchase order is one order. Filter by stage — some are only created, others confirmed, delivered, or invoiced.
        </p>
      </div>
