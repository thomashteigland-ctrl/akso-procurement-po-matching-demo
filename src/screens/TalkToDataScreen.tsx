...
const SUGGESTIONS = [
  'What’s our total spend with Nordisk Stål?',
  'Total spend by supplier',
  'Order volume per supplier',
  'Average unit price per product per supplier',
]
...
export function TalkToDataScreen() {
  ...
  function ask(question: string) {
    ...
    if (kind === 'unknown') {
      addChat('assistant', 'Try spend, order volume, or average unit price.')
    } else {
  ...
  return (
    <div className="grid h-full min-h-0 grid-cols-1 gap-3 p-4 lg:grid-cols-[16.5rem_minmax(0,1fr)_20rem]">
      ...
      <section className="flex min-h-0 flex-col rounded-xl border border-line bg-white p-4">
        ...
        {completed.length > 0 && askDraft && !selectedGraph && !selectedDashboard && (
        ...
        {completed.length > 0 && selectedGraph && (
          <DataChart graph={selectedGraph} completed={completed} />
        )}
        {completed.length > 0 && selectedDashboard && (
              ...
              <div className="grid gap-4 md:grid-cols-2">
                {dashboardGraphs.map((graph) => (
                  <button
                    ...
                    className="rounded-lg border border-line p-3 text-left hover:border-teal"
                  >
                    <DataChart graph={graph} completed={completed} compact />
                  </button>
                ))}
              </div>
            )}
